...
private bool m_operational = false;
  ...
  private static readonly int ATTRIBUTE_IX_NAMESPACE_URI = 0;
    ...
    private static readonly int CHUNK_XML_LAST = 1048836; private IntReader m_reader;
...
public int getAttributeNameResource(int index)
{
    ...
    if ((this.m_resourceIDs == null) ||
      (name < 0) || (name >= this.m_resourceIDs.length))
    {
        return 0;
    }
    ...
    throws XmlPullParserException
{
  ...
  private static readonly class NamespaceStack
{
    private int[] m_data;
    ...
    public readonly int getTotalCount()
    {
        return this.m_count;
    }
    ...
    public readonly int getAccumulatedCount(int depth)
    {
    ...
    public readonly void push(int prefix, int uri)
    {
        if (this.m_depth == 0)
        {
            increaseDepth();
    ...
    public readonly bool pop(int prefix, int uri)
    {
        if (this.m_dataLength == 0)
        {
            return false;
    ...
    public readonly int getPrefix(int index)
    {
        return get(index, true);
    }

    public readonly int getUri(int index)
    {
        return get(index, false);
    }

    public readonly int findPrefix(int uri)
    {
        return find(uri, false);
    }

    public readonly int findUri(int prefix)
    {
    ...
    public readonly void increaseDepth()
    {
        ensureDataCapacity(2);
    ...
    public readonly void decreaseDepth()
    {
        if (this.m_dataLength == 0)
        {
            return;
    ...
    private readonly int find(int prefixOrUri, bool prefix)
    {
        if (this.m_dataLength == 0)
        {
            return -1;
    ...
    private readonly int get(int index, bool prefix)
    {
...
readonly StringBlock getStrings()
{
    return this.m_strings;
}

private readonly int getAttributeOffset(int index)
{
    if (this.m_event != 2)
    {
...
private readonly int findAttribute(String namespace, String attribute)
  {
    ...
    int uri = namespace != null ?
      this.m_strings.find(namespace) : 
      -1;
    for (int o = 0; o != this.m_attributes.length; o++) {
    if ((name == this.m_attributes[(o + 1)]) && (
      (uri == -1) || (uri == this.m_attributes[(o + 0)])))
    {
        return o / 5;
    }
}
...
private readonly void resetEventInfo()
{
    this.m_event = -1;
    this.m_lineNumber = -1;
    this.m_name = -1;
    ...
    throws IOException
{
    ...
    do
    {
        ...
        for (; ; )
        {
            ...
            if ((event == 3) &&
              (this.m_namespaces.getDepth() == 1) &&
              (this.m_namespaces.getCurrentCount() == 0))
        {
            this.m_event = 1; return;
        }
        int chunkType;
        if (event == 0) {
            chunkType = 1048834;
